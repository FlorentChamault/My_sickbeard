...
class PostProcessor(object):
    ...
    def _list_dummy_files(self, file_path, oribasename=None,directory=None):
        ...
        if not base_name:
            return []
        logger.log(u"DEBUG 0 "+str(base_name), logger.ERROR) 
        # don't confuse glob with chars we didn't mean to use
        base_name = re.sub(r'[\[\]\*\?]', r'[\g<0>]', base_name)
        logger.log(u"DEBUG 1 "+str(base_name), logger.ERROR)
        if directory =="d":
            cur_dir=file_path
        else:
    ...
    def _analyze_name(self, name, file=True):
        ...
        if not name:
            return to_return
        logger.log(u"DEBUG 21"+str(name), logger.ERROR) 
        # parse the name to break it into show name, season, and episode
        np = NameParser(file)
#        logger.log(u"DEBUG 22"+str(np), logger.ERROR)
        parse_result = np.parse(name)
        logger.log(u"DEBUG 23"+str(parse_result), logger.ERROR)
        self._log("Parsed "+name+" into "+str(parse_result).decode('utf-8'), logger.DEBUG)

        if parse_result.air_by_date:
            season = -1
            episodes = [parse_result.air_date]
#            logger.log(u"DEBUG 24"+str(episodes), logger.ERROR)
        else:
        ...
        def _finalize(parse_result):
            ...
            if parse_result.extra_info:
                ...
                #logger.log(u"DEBUG 25"+str(self.is_proper, logger.ERROR)
